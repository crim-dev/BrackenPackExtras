...
execute if entity @s[scores={bp.tick=2}] if entity @e[type=minecraft:eye_of_ender,distance=..4,nbt={Item:{components:{"minecraft:custom_data":{bracken:{id:"enderling_controlled_teleport"}}}}}] run function bracken:player/species/enderling/tp/controlled_teleport
